...
export default function EstadisticasPage() {
  ...
  const topServices = [
    { name: 'Balayage Premium', category: 'Cabello', revenue: '$3,600', appointments: 30, growth: '+15%' },
    { name: 'Manicure Gel + Decoración', category: 'Uñas', revenue: '$1,250', appointments: 50, growth: '+5%' },
    { name: 'Maquillaje para Eventos', category: 'Maquillaje', revenue: '$825', appointments: 15, growth: '+22%' },
    { name: 'Corte Caballero', category: 'Barba', revenue: '$450', appointments: 30, growth: '-2%' },
  ];
    ...
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav className="flex text-sm text-gray-500 dark:text-text-muted mb-2">
            ...
            <span className="text-gray-900 font-medium dark:text-primary">Estadísticas</span>
          </nav>
          <h2 className="text-3xl font-bold tracking-tight text-cream-label">Análisis y Estadísticas</h2>
          <p className="text-text-muted mt-1">Métricas detalladas de crecimiento para Michell's Beauty en Santa Ana.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-border-dark bg-surface-dark p-6">
          <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1">Ingresos Brutos</p>
          <div className="flex items-end gap-3">
            <h3 className="text-3xl font-bold text-cream-label">$8,450</h3>
            <span className="text-sm font-bold text-green-500 mb-1">+12.5%</span>
          </div>
        </div>
        <div className="rounded-xl border border-border-dark bg-surface-dark p-6">
          <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1">Ticket Promedio</p>
          <div className="flex items-end gap-3">
        ...
        <div className="rounded-xl border border-border-dark bg-surface-dark p-6">
          <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1">Citas Completadas</p>
          <div className="flex items-end gap-3">
        ...
        <div className="rounded-xl border border-border-dark bg-surface-dark p-6">
          <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1">Cancelaciones</p>
          <div className="flex items-end gap-3">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-border-dark bg-surface-dark overflow-hidden flex flex-col">
          <div className="p-6 border-b border-border-dark">
            <h3 className="font-bold text-cream-label">Horas de Mayor Demanda</h3>
            <p className="text-xs text-text-muted mt-1">Tráfico de clientes durante el día</p>
          </div>
          <div className="flex-1 p-6 relative flex items-end justify-between gap-1 h-[250px]" style={chartGridStyle}>
            {/* Barras representando las horas del día de 8am a 6pm */}
            {[20, 45, 80, 60, 40, 50, 90, 100, 70, 30].map((height, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                <div className="w-full max-w-[24px] bg-primary/20 rounded-t-sm hover:bg-primary transition-all relative flex items-end justify-center" style={{ height: `${height}%` }}>
                  ...
                  <div className="w-full bg-primary/40 rounded-t-sm" style={{ height: `${height}%` }}></div>
                </div>
        ...
        <div className="rounded-xl border border-border-dark bg-surface-dark p-6 flex flex-col">
          <div className="mb-6">
            <h3 className="font-bold text-cream-label">Retención de Clientes</h3>
            <p className="text-xs text-text-muted mt-1">Nuevos vs. Recurrentes este mes</p>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center space-y-8">
            ...
            <div>
              ...
              <div className="h-4 bg-luxury-black rounded-full overflow-hidden border border-border-dark relative">
                <div className="absolute top-0 left-0 h-full bg-primary rounded-full" style={{ width: '68%' }}></div>
              </div>
              <p className="text-xs text-text-muted mt-2 text-right">88 clientes</p>
            </div>
            ...
            <div>
              ...
              <div className="h-4 bg-luxury-black rounded-full overflow-hidden border border-border-dark relative">
                <div className="absolute top-0 left-0 h-full bg-[#cbbc90] rounded-full" style={{ width: '32%' }}></div>
              </div>
              <p className="text-xs text-text-muted mt-2 text-right">42 clientes</p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border-dark bg-surface-dark overflow-hidden">
        <div className="p-6 border-b border-border-dark flex justify-between items-center">
          <div>
            <h3 className="font-bold text-cream-label">Servicios Top por Ingresos</h3>
            <p className="text-xs text-text-muted mt-1">Los tratamientos más rentables del mes.</p>
          </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-surface-input text-xs uppercase text-primary border-b border-border-dark">
              <tr>
                ...
                <th className="px-6 py-4 font-bold tracking-wider text-center">Citas Totales</th>
                <th className="px-6 py-4 font-bold tracking-wider text-right">Ingresos</th>
                <th className="px-6 py-4 font-bold tracking-wider text-right">Crecimiento</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border-dark">
              {topServices.map((service, index) => (
                <tr key={index} className="hover:bg-surface-input/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-bold text-cream-label flex items-center gap-2">
                      {index === 0 && <span className="material-symbols-outlined text-primary text-[18px]">military_tech</span>}
                      {service.name}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-center font-medium text-text-muted">{service.appointments}</td>
                  <td className="px-6 py-4 text-right font-bold text-cream-label">{service.revenue}</td>
                  <td className={`px-6 py-4 text-right font-bold ${service.growth.startsWith('+') ? 'text-green-500' : 'text-[#ef4444]'}`}>
                    {service.growth}
                  </td>
                </tr>
              ))}
